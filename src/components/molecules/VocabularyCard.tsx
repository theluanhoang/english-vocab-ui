...
import DetailVocabularySection from './DetailedVocabularySection';
...
function VocabularyCard({ vocabulary, isOpen, volumes, toggleAccordion, idx, handleVolumeChange }: VocabularyCardProps) {
        ...
        >
            ...
            >
                <div className="flex items-center gap-3 md:gap-4 text-left w-full md:w-auto">
                    ...
                    <div>
                        <Heading as='h3' className="text-xl md:text-2xl font-bold text-content-primary-light dark:text-content-primary-dark">{vocabulary.word}</Heading>
                        <span className={`inline-block px-3 py-1 text-sm font-semibold rounded-full mt-1 shadow-sm border border-transparent ${getTagColor(vocabulary.partOfSpeech)}`}>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="w-24 flex items-center px-2 py-1.5 bg-background-slider-light dark:bg-background-slider-dark rounded-lg group">
                        {(() => {
                            const percent = (volumes[vocabulary.id] ?? 1) * 100;
                            ...
                            return (
                                <VolumeBar
                                    value={volumes[vocabulary.id] ?? 1}
                                    formId={vocabulary.id}
            ...
            {isOpen && (
                <DetailVocabularySection vocabulary={vocabulary} />
            )}
        </div>
    )
}
